...
def test_a_standalone_stim_server_reports_screen_errors_to_its_client():
    ...
    try:
        ...
        if not wait_until(arrived, timeout=15):
            ...
            # screen that cannot create a context dies, and the socket to it breaks. (On macOS
            # the launcher swaps an inherited offscreen platform for cocoa -- see launch_screen --
            # so a screen that still cannot paint is a machine without WindowServer access, and
            # it dies into this same branch.)
