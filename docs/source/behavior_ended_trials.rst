===============================================
Trials that end when the subject does something
===============================================

Most protocols set a stimulus duration and every trial lasts that long. Some experiments need the
opposite: the trial ends when the subject fixates long enough, reaches a virtual goal, makes a
choice, or stops walking. This page is about how stimpack supports that, and about the one thing
it changes in your data.

.. tip::

   This page has two runnable examples that need no hardware, with tracking pre-checked.
   ``ReachTheGoal``: press View, hold the Up arrow in the KeyTrac window, and the trial ends as you
...
The obvious approach -- have the protocol watch the subject and stop when it likes -- cannot work,
...
(:doc:`overview`), so there is no "where is the subject?" call to make. This is not an oversight
...
.. important::
    ...
    the subject crosses your threshold and the run ends on that same update, it never fires at
...
hardware trigger, and it is the slowest of the ways a stimulus can respond to a subject.
...
``end_trial`` is for one thing: ending the trial. Making the *stimulus* respond to the subject is a
...
.. list-table::
    ...
    * - What you want
      - Where it happens
    * - The scene tracks the subject's position and heading
      - Automatic. Every ``paintGL``, the screen recomputes its perspective matrix from the current
        subject position, so the viewpoint follows the subject with no code of your own.
    ...
    * - The stimulus itself reshapes per frame based on where the subject is
      - A custom stimulus. ``paintGL`` passes ``subject_position`` into every stimulus's
        ``eval_at``, so it can rebuild its geometry each frame from the subject's current state.
    * - The trial ends
      - ``end_trial`` -- this page.
...
So if a stimulus is not responding quickly enough to the subject, ``end_trial`` is not the tool that
...
1.4 s" and "this trial was 1.4 s because the subject reached the goal", which is usually the thing
...
Ending a trial once the subject has held its heading within 10 degrees for half a second:
...
.. note::
    ...
    update reaching the screens, which is the closed-loop path the subject is actually in.
