...
class GenOrthographic:
    ...
    def matrix(self):
        ...
        mat =  np.array([[2/(r-l),        0,         0,         (l+r)/(l-r)],
                       [        0,  2/(t-b),         0,         (b+t)/(b-t)],
                       [        0,         0,  2/(n-f),         (n+f)/(n-f)],
                       [        0,         0,        0,                   1]], dtype=float)

        return mat.astype('f4').tobytes(order='F')
    
    def rotx(self, th):
        return GenOrthographic(pa=rotx(self.pa, th), pb=rotx(self.pb, th), pc=rotx(self.pc, th),
                              pe=rotx(self.pe, th), near=self.near, far=self.far, subject_xyz=self.subject_xyz, horizontal_flip=self.horizontal_flip)

    def roty(self, th):
        return GenOrthographic(pa=roty(self.pa, th), pb=roty(self.pb, th), pc=roty(self.pc, th),
                              pe=roty(self.pe, th), near=self.near, far=self.far, subject_xyz=self.subject_xyz, horizontal_flip=self.horizontal_flip)

    def rotz(self, th):
        return GenOrthographic(pa=rotz(self.pa, th), pb=rotz(self.pb, th), pc=rotz(self.pc, th),
