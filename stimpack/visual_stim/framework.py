...
class StimDisplay(QOpenGLWidget):
    ...
    def paint_subframe(self, time_offset, display_width, display_height):
        ...
        # Drawn per subframe, not per frame. The photodiode is what says when a frame appeared, so
        # under multiplexing it has to mark the subframes -- otherwise it reports 120 Hz for a
        # display running at 360 and there is no way to check the timing that matters.
